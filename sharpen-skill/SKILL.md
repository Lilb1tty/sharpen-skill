...
4. Turn each supported gap into a scoped change at the smallest appropriate location. A repeated `brainstorming → grill-me → spec` sequence, for example, may justify a cue to offer `grill-me` before drafting the spec, rather than making that step mandatory.
