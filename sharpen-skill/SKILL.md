...
3. Express the gap as a scoped decision rule. Distinguish a recurring preference from a one-time constraint, and distinguish the next step's cue from a requirement to run that step every time. For example, a repeated `brainstorming → grill-me → spec` sequence may warrant a prompt at the end of brainstorming to offer `grill-me` before drafting the spec, while leaving the user's choice intact.
